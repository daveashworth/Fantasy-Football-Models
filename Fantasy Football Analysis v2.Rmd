...
projections2023 <- read_csv("https://raw.githubusercontent.com/jsoslow2/Fantasy-Football-Models/master/data/projections/2023%20projections%20-%20NFL23_CS_PPR300.csv.csv")


depthChart2023 <- read_csv("https://raw.githubusercontent.com/jsoslow2/Fantasy-Football-Models/master/data/depth%20charts/2023%20depth%20charts%20-%20NFL23_CS_Depth.csv.csv")
...
#2023
projections2023$Name <- ""
projections2023$team <- ""
projections2023$position <- ""
split_strings <- str_split(projections2023$`name team`, ",")
split_strings2 <- str_split(projections2023$`position rank`, " ")
i = 1
for (i in 1:nrow(projections2023)) {
  name <- split_strings[[i]][1]
  team <- split_strings[[i]][2]
  team <- str_sub(team, 2, -1)
  ...
  projections2023$Name[i] <- name
  projections2023$team[i] <- team
  projections2023$position[i] <- position
}
projections2023$rank <- seq.int(nrow(projections2023))
projections2023$year <- 2023
...
projections2023 <- projections2023 %>% 
  select(Name, team, position, year, rank)


allProjections <- bind_rows(projections2010, projections2012, projections2013, projections2014, projections2015, projections2016, projections2017, projections2018, projections2019, projections2020, projections2021, projections2022, projections2023)
```
...
#2023
depthChart2023$Name <- ""
splitty <- str_split(depthChart2023$`name rank`, "[[(]]")
i <- 1
for (i in 1:length(splitty)) {
  name <- splitty[[i]][1]
  name <- str_sub(name, 1, -2)
  depthChart2023$Name[i] <- name
}
depthChart2023$year <- 2023
...
depthChartsAll <- bind_rows(depthChart2023, depthChart2022, depthChart2021, depthChart2020, depthChart2019, depthChart2018, depthChart2017, depthChart2016, depthChart2015, depthChart2014, depthChart2012, depthChart2010)
```
...
roster_2023 <- load_rosters(2023) %>% select(roster_columns)
```
...
pbp_2022 <- load_pbp(2022) %>% select(pbp_columns) %>% mutate(Season = 2022)
```
...
calc_passing_splits <- function(splits,pbp_df) {
  ...
  pass_output <- pbp_df %>% group_by_(.dots=split_groups) %>%
    dplyr::summarise(Player_Name = find_player_name(passer_player_name[which(!is.na(passer_player_name))]),
              ...
              airWPA_Comp = sum(complete_pass*air_wpa,na.rm = TRUE),
...
# Is 2023 supposed to be in here or is this only for previous years?
rosters <- bind_rows(roster_2009, roster_2010, roster_2011, roster_2012,
                 roster_2013, roster_2014, roster_2015, roster_2016,
                 roster_2017, roster_2018, roster_2019, roster_2020, roster_2021, roster_2022, roster_2023) %>% 
...
```{r}
allData %>% 
  filter(grepl("Dak", full_player_name)) %>% 
...
#Find the people who have the fucking same name in the same year LOL
...
# TODO Not sure what this line does or if it needs to be updated for the current year
...
```{r}
draftedOverall <- c(
  'justinjefferson','christianmccaffrey','tyreekhill','traviskelce','jamarrchase','austinekeler','cooperkupp','stefondiggs','bijanrobinson','ceedeelamb',
  'saquonbarkley','amonrast','derrickhenry','patrickmahomes','davanteadams','ajbrown','jonathantaylor','nickchubb','jaylenwaddle','garrettwilson',
  'joshjacobs','joshallen','rhamondrestevenson','teehiggins','jalenhurts','dkmetcalf','chrisolave','devontasmith','markandrews','tonypollard',
  'deebosamuel','breecehall','joemixon','najeeharris','travisetienne','aaronjones','joeburrow','jahmyrgibbs','keenanallen','calvinridley',
  'kennethwalker','deandrehopkins','lamarjackson','drakelondon','amaricooper','tjhockenson','djmoore','chrisgodwin','justinfields','georgekittle',
  'terrymclaurin','dameonpierce','jerryjeudy','milessanders','alexandermattison','dallasgoedert','christianwatson','alvinkamara','dandreswift','trevorlawrence',
  'kylepitts','darrenwaller','jkdobbins','camakers','michaelpittman','justinherbert','dalvincook','jamesconner','brandonaiyuk','christiankirk',
  'dakprescott','marquisebrown','mikeevans','jordanaddison','tylerlockett','rachaadwhite','mikewilliams','davidmontgomery','isiahpacheco','javontewilliams',
  'ajdillon','jaxonsmithnjigba','evanengram','georgepickens','jamescook','diontaejohnson','davidnjoku','daltonschultz','brandincooks','jahandotson',
  'kadariustoney','michaelthomas','danieljones','treylonburks','antoniogibson','brianrobinson','elijahmoore','quentinjohnston','zachcharbonnet','deshaunwatson',
  'jujusmithschuster','patfreiermuth','gabedavis','tuatagovailoa','jamaalwilliams','zayflowers','kirkcousins','genosmith','daltonkincaid','samajeperine',
  'aaronrodgers','anthonyrichardson'
)

yourTeam <- c(
  'ceedeelamb','derrickhenry','devontasmith','joemixon','chrisgodwin','jerryjeudy','jamesconner','mikeevans','daltonschultz','danieljones','genosmith'
)
...
available <- copy[!tolower(copy$Name) %in% tolower(draftedOverall),] %>% group_by(position) %>%  arrange(rank) %>% dplyr::slice(1:100)
...
```{r}
write_csv(secondDataframe, "ranks_2023.csv")
```
